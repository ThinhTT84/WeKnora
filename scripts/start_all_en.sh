#!/bin/bash
# This script is used to start/stop Ollama and docker-compose services as needed
...
# Get project root directory (one level up from script directory)
...
show_help() {
    ...
    echo "  --no-pull      Don't pull images on startup (not pulling is default)"
...
# Install Ollama (different methods depending on platform)
install_ollama() {
    # Check if it's a remote service
    get_ollama_base_url
    
    if [ $IS_REMOTE -eq 1 ]; then
        log_info "Remote Ollama service configuration detected, no need for local installation"
        return 0
    fi
    ...
    if [ "$OS" = "Darwin" ]; then
        ...
        log_info "Mac system detected, installing Ollama via brew..."
        if ! command -v brew &> /dev/null; then
            # Install via package
            log_info "Homebrew not installed, downloading directly..."
...
# Get Ollama base URL, check if remote service
get_ollama_base_url() {
...
start_ollama() {
    ...
    if [ $IS_REMOTE -eq 1 ]; then
        log_info "Remote Ollama service detected, will use remote service directly, bypassing local install and start"
        # Check if remote service is accessible
        if curl -s "$OLLAMA_URL/api/tags" &> /dev/null; then
            log_success "Remote Ollama service is accessible"
            return 0
        else
            log_warning "Remote Ollama service is not accessible, please confirm address is correct and service is running"
            return 1
        fi
    fi
    ...
    if curl -s "http://localhost:$OLLAMA_PORT/api/tags" &> /dev/null; then
        ...
        # Note: Official recommendation is systemctl or launchctl, direct background run is for temporary use
        ...
        while [ $COUNT -lt $MAX_RETRIES ]; do
            if curl -s "http://localhost:$OLLAMA_PORT/api/tags" &> /dev/null; then
                log_success "Local Ollama service started successfully on port: $OLLAMA_PORT"
                break
            fi
...
check_docker() {
	...
	if detect_compose_cmd; then
		...
		log_error "Docker Compose not detected (neither 'docker compose' nor 'docker-compose'). Please install one."
		return 1
	fi
...
check_platform() {
    ...
    log_info "Detecting system platform info..."
...
# Pre-pull sandbox image (required for Agent Skills, only pulls, doesn't start)
ensure_sandbox_image() {
    ...
    log_info "Agent Skills features depend on this image, initial pull needed before execution"

    # Pull in background, don't block main process
    (
        if PLATFORM=$PLATFORM "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD --profile sandbox pull sandbox 2>/dev/null; then
            ...
            log_warning "Sandbox image pull failed, Agent Skills features might be unavailable"
...
start_docker() {
    ...
    # Enter project root then execute docker-compose commands
    cd "$PROJECT_ROOT"
    
    # Start basic services
    log_info "Starting core service containers..."
	# Uniform startup via detected Compose command
	if [ "$NO_PULL" = true ]; then
		# Don't pull images, use local ones
		...
		PLATFORM=$PLATFORM "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD --profile full up --pull always -d 
	fi
    if [ $? -ne 0 ]; then
...
stop_docker() {
    ...
    check_docker
    if [ $? -ne 0 ]; then
        # Try stopping anyway just in case
    ...
    # Stop all containers
	"$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD --profile full down --remove-orphans
    if [ $? -ne 0 ]; then
...
pull_images() {
    ...
    # Pull sandbox image (sandbox is in a profile, needs separate pull)
    ...
    log_success "All images successfully pulled to latest version"
...
check_environment() {
    ...
    if [ $IS_REMOTE -eq 1 ]; then
        ...
        if curl -s "$OLLAMA_URL/api/tags" &> /dev/null; then
            ...
            log_success "Remote Ollama service accessible, version: $version"
        else
            log_warning "Remote Ollama service not accessible, please confirm address is correct and service is running"
        fi
    else
        if command -v ollama &> /dev/null; then
            log_success "Local Ollama installed"
            if curl -s "http://localhost:$OLLAMA_PORT/api/tags" &> /dev/null; then
                version=$(curl -s "http://localhost:$OLLAMA_PORT/api/tags" | grep -o '"version":"[^"]*"' | cut -d'"' -f4)
                log_success "Local Ollama service running, version: $version"
            else
                log_warning "Local Ollama installed but service not running"
            fi
        else
            log_warning "Local Ollama not installed"
        fi
    fi
    ...
    if docker image inspect "$sandbox_image" &> /dev/null; then
        ...
        log_warning "Sandbox image not found: $sandbox_image (Required for Agent Skills features)"
        log_info "Can pull via: $0 -p or docker pull $sandbox_image"
    fi
    ...
    if docker info &> /dev/null; then
        ...
        log_warning "Cannot get container status, Docker might not be running"
    fi
...
# Default to starting all services if no arguments provided
if [ $# -eq 0 ]; then
    START_OLLAMA=false
    START_DOCKER=true
fi
...
if [ "$STOP_SERVICES" = true ]; then
    ...
    if [ "$START_OLLAMA" = true ] && [ "$START_DOCKER" = true ]; then
        if [ $OLLAMA_RESULT -eq 0 ] && [ $DOCKER_RESULT -eq 0 ]; then
            ...
            echo ""
            log_info "Continuously outputting container logs (Press Ctrl+C to exit logs, containers won't stop)..."
